...
HOTEL_IMAGES = {
    "front": "https://raw.githubusercontent.com/yrskaran/ganga-palace-bot/main/images/main.jpg",
...
def ask_cohere(user_message):
    ...
    payload = {
        ...
        "preamble": (
            "You are the polite AI Receptionist for Hotel Ganga View, Haridwar (Near Har Ki Pauri). "
            "Tariff: Standard Room ₹1,800/night, Deluxe AC Room ₹2,500/night. "
            "Reply strictly in 1-2 lines in easy Hinglish."
        ),
        "temperature": 0.1
    }
    try:
...
def process_and_reply(user_text, sender_phone):
    print(f"[PROCESS] '{user_text}' from {sender_phone}", flush=True)
    ...
    # 4. AI & SMART FALLBACK
    cohere_reply = ask_cohere(f"Guest: {user_text}")
    if cohere_reply:
        send_whatsapp_message(sender_phone, cohere_reply)
    else:
        if not guest_info:
            send_whatsapp_message(
                sender_phone,
                "Ji Namaste! Hamare paas Standard (₹1,800) aur Deluxe AC (₹2,500) rooms uplabdh hain. Photos dekhne ke liye 'Room photo' likhein ya dates batayein! 🙏"
            )
        else:
            send_whatsapp_message(
                sender_phone,
                f"Ji {guest_info['name']} ji, batayein mai aapki kya seva kar sakta hoon? Khana order karna hai ya bill check karna hai?"
            )
