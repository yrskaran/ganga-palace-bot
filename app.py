...
# ==========================================
# 1. ENVIRONMENT CONFIGURATION & GLOBALS
# ==========================================
...
def get_guest_stay_status(sender_phone):
    ...
    try:
        ...
        for row in reader:
            sheet_phone = re.sub(r"\D", "", str(row.get("Phone", "")))
            status = str(row.get("Status", "")).strip().upper()
            
            # Match registered mobile with status CHECKED_IN
            if sheet_phone == clean_sender and status == "CHECKED_IN":
                return {
                    "is_inhouse": True,
...
def process_and_reply(user_text, sender_phone):
    ...
    # Check if outside guest is requesting service orders
    ...
    # Reject in-house requests if not checked in
    ...
    # Attach verified room context to message for LLM
